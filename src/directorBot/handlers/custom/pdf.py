...
from src.database.pdf_func import generate_pdf_report_for_all_staff, generate_pdf_report_person
...
async def busy_init(message: Message, state: FSMContext):
    ...
    await message.reply('Выберите необходимый пункт меню', reply_markup= rep.employee)
...
async def busy_next(message: Message, state: FSMContext):
    ...
    await message.reply('Отчет на всех сотрудников готов', reply_markup=ReplyKeyboardRemove())
    # await message.reply_document(reply_markup=ReplyKeyboardRemove(), document=)
...
async def busy_person_answer(message:Message, state: FSMContext):
    ...
    await generate_pdf_report_person(name= name.lower(), surname= surname.lower())
    await message.reply(f'Отчет на {name}, {surname}', reply_markup=ReplyKeyboardRemove())
    # await message.reply_document(reply_markup=ReplyKeyboardRemove(), document=)

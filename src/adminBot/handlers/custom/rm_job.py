...
from src.database.data_func import rm_job
...
async def add_dir_init(message: Message, state: FSMContext):
    ...
    await message.reply('Для удаления из базы данных, выберите вид работы из списка', reply_markup= await rep.check_job())
...
async def add_dir_choice(message: Message, state: FSMContext):
    ...
    await message.reply(f'Вид работ: "{message.text}" удален из базы данных, '
                        f'старые заявки с удаленным видом работ всё ещё доступны', reply_markup=ReplyKeyboardRemove())
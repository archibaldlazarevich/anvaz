...
async def get_db_log(message: Message, state: FSMContext):
    await state.clear()
    file = FSInputFile(
        path=("base.db"), filename="base.db"
    )
    log_file = FSInputFile(
        path=(f"output.log"), filename=f"output.log"
    )
    await message.answer_document(document=file, caption="Текущая база данных.", reply_markup=ReplyKeyboardRemove())
    await message.answer_document(document=log_file, caption='Текущий файл логов.')
